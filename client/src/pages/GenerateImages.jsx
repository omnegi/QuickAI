...
const GenerateImages = () => {
     ...
     const imageStyle=['Realistic', 'Cartoon style','Potrait style', '3D style', 'Fantasy style','Anime style','Ghibli style'];
  ...
  const { getToken } = useAuth();
      ...
      const onSubmitHandler = async(e) => {
        ...
        try {
          setLoading(true);
          const prompt = `Generate an image in the style of ${selectedStyle} with the description: "${input}"`;
  ...
  return (
   <div className='p-6 h-full overflow-y-scroll flex items-start flex-wrap gap-4 text-slate-700'>
      <form onSubmit={onSubmitHandler}  className='w-full max-w-lg p-4 bg-white rouded-lg border border-gray-200' action="">
